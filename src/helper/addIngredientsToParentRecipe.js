    ...
    recipe.nestedRecipes.forEach(includedRecipeReference => {
        ...
        includedRecipe.ingredients.forEach(ingredient => {
            ...
            for (let parentRecipeIngredient of recipe.ingredients) {
                if (parentRecipeIngredient.id === ingredient.id && parentRecipeIngredient.isFlour === ingredient.isFlour && parentRecipeIngredient.isLiquid === ingredient.isLiquid) {
                    found = true
                    break
                }
            }
            if (!found) {
                recipe.ingredients.push(new RecipeIngredient(ingredient.isFlour, ingredient.isLiquid, ingredient.id, 0))
            }
        })
    })
    return recipe
