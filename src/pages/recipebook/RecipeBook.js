...
function RecipeBook() {
    ...
    return (
        <>
            <div className="tabs">
                <nav className={showRecipes ? 'tabs__item --active' : 'tabs__item'}
                ...
                {showRecipes &&
                    <ul className="recipes-ingredients-list">
                        {recipesByCategory?.map((category, index) =>
                            <RecipesAccordion key={index} category={category} recipeBook={recipeBook}/>
                        )}
                    </ul>
                }
                {!showRecipes &&
                    <ul className="recipes-ingredients-list">
                        {ingredientsByCategory?.map((category, index) =>
                            <IngredientsAccordion key={index} category={category} recipeBook={recipeBook}/>
                        )}
                    </ul>
                }
